package view.constants;

public class MultiplicationConstants {
    public static final String TITLE = "\n" + "[!] Interfaz de multiplicación de 2 polinomios ";
    public static final String SUBTITLE = 
    "[*] Escoja 2 polinomios de la lista. \n ingrese \"0\" para salir sin hacer la operación ";

    public static final String NOTES = "[*] Ingrese un numero entre 1 y ";
    public static final String QUESTION = "[?] Ingresar polinomio : ";
    public static final String ERROR = "Debe ingresar un numero valido";
    public static final String ERROR_EMPTY = "Aun no hay ningún polinomio guardado, debe ingresar uno primero";
    public static final String RESULT = "El resultado de la multiplicación es:";
    public static final String OUT = "Presione \".\" para salir ";

}
